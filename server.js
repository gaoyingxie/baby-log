...
async function doUpdate(res) {
  try {
    ...
    const extractDir = '/tmp/baby-log-extract';
    ...
    const srcDir = findServerJs(extractDir);
    if (!srcDir) throw new Error('未找到 server.js');
    console.log('📦 解压到:', srcDir);

    execSync('cp ' + srcDir + '/server.js /app/server.js');
    execSync('mkdir -p /app/public && cp -r ' + srcDir + '/public/* /app/public/ 2>/dev/null; true');
